...
import { ArrowDownCircle, BookOpen, ChevronRight, FileText, Search } from 'lucide-react';
...
export function KomponenBelanjaPage() {
  ...
  const kodeAnggaranBelanjaFromDoorscrieft = useMemo(() => {
    ...
    doorscrieftTransaksis
      ...
      .forEach((r) => {
        ...
        current.total += Number(r.pengeluaran || 0);
        current.count += 1;
  ...
  const summary = useMemo(() => {
    const total = kodeAnggaranBelanjaFromDoorscrieft.reduce((sum, item) => sum + item.total, 0);
    const count = kodeAnggaranBelanjaFromDoorscrieft.reduce((sum, item) => sum + item.count, 0);
    const top = kodeAnggaranBelanjaFromDoorscrieft.length > 0
      ? [...kodeAnggaranBelanjaFromDoorscrieft].sort((a, b) => b.total - a.total)[0]
      : null;
    return { total, count, top };
  }, [kodeAnggaranBelanjaFromDoorscrieft]);
    ...
    <div className='space-y-6'>
      <div className='flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between'>
        <div>
          ...
          <h1 className='mt-1 text-2xl font-bold text-slate-900 dark:text-white'>Komponen Pengeluaran</h1>
          <p className='text-slate-500 dark:text-slate-400'>Mata anggaran pengeluaran yang terpakai di Doorscrieft Tahun {tahunAktif}</p>
        </div>
        <ReportPrintButton title={`Komponen_Pengeluaran_${tahunAktif}`} />
      </div>

      <ReportPrintDocument
        title='KOMPONEN PENGELUARAN'
        subtitle={`Tahun Anggaran ${tahunAktif}`}
        meta={[
          { label: 'Tahun', value: tahunAktif },
          { label: 'Kode Aktif', value: kodeAnggaranBelanjaFromDoorscrieft.length },
          { label: 'Transaksi', value: summary.count },
          { label: 'Total Pengeluaran', value: formatCurrency(summary.total) },
        ]}
      >
        <table>
          <thead>
          ...
          <tbody>
            {filteredKodeAnggaran.length === 0 ? (
              <tr><td colSpan={5} className='text-center'>Tidak ada komponen pengeluaran.</td></tr>
      ...
      <div className='grid gap-3 md:grid-cols-3'>
        <Card>
          <CardContent className='p-4'>
            ...
            <p className='mt-2 text-xl font-bold text-rose-700 dark:text-rose-300'>{formatCurrency(summary.total)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className='p-4'>
            <p className='text-sm text-slate-500 dark:text-slate-400'>Kode Aktif</p>
            <p className='mt-2 text-xl font-bold text-slate-900 dark:text-white'>{kodeAnggaranBelanjaFromDoorscrieft.length}</p>
      ...
      <div className='grid min-h-0 gap-4 xl:grid-cols-[420px_minmax(0,1fr)]'>
        <Card className='flex max-h-[calc(100vh-13rem)] min-h-[560px] flex-col overflow-hidden'>
          <CardHeader>
            ...
            <p className='text-sm text-slate-500 dark:text-slate-400'>Pilih kode untuk melihat transaksi pengeluaran.</p>
          </CardHeader>
            ...
            <div className='min-h-0 flex-1 overflow-y-auto pr-1'>
              {filteredKodeAnggaran.length === 0 ? (
                <div className='flex h-full flex-col items-center justify-center gap-2 text-center text-sm text-slate-500 dark:text-slate-400'>
                  ...
                  <p>Tidak ada mata anggaran pengeluaran untuk filter ini.</p>
                </div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className='mt-2 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400'>
                          ...
                          <span className='font-semibold text-rose-700 dark:text-rose-300'>{formatCurrency(k.total)}</span>
                        </div>
                      </button>
                    );
                  })}
            ...
            <div>
              ...
              <p className='mt-1 text-sm text-slate-500 dark:text-slate-400'>
                {kodeAktifItem
                  ...
                  : 'Pilih mata anggaran pengeluaran.'}
              </p>
            </div>
            <div className='text-right'>
              <p className='text-sm text-slate-500 dark:text-slate-400'>Total Pengeluaran</p>
              <p className='text-xl font-bold text-rose-700 dark:text-rose-300'>{formatCurrency(totals.pengeluaran)}</p>
            </div>
          </CardHeader>
                ...
                <table className='w-full min-w-[720px] text-sm'>
                  ...
                  <tbody className='divide-y divide-slate-100 dark:divide-slate-700'>
                    {rowsBelanjaAktif.length === 0 ? (
                      <tr>
                        <td colSpan={4} className='px-4 py-10 text-center text-slate-500'>Tidak ada transaksi pengeluaran.</td>
                      </tr>
                    ) : (
