...
import { ArrowDownCircle, CalendarDays, FileText, Search } from 'lucide-react';
...
export function PengeluaranPerbulanPage() {
  ...
  useEffect(() => {
    ...
    const filledMonth = getLatestFilledMonth(doorscrieftTransaksis, tahunAktif, isBelanjaRow);
  ...
  const rawMonthTotal = monthlyRows.reduce((s, row) => s + Number(row.pengeluaran || 0), 0);
  ...
  const emptyText = search.trim()
    ...
    : `Tidak ada data pengeluaran untuk ${MONTHS[selectedBulan - 1]} ${tahunAktif}.`;
    ...
    <div className='space-y-6 pb-24'>
      <div className='flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between'>
        <div>
          ...
          <h1 className='mt-1 text-2xl font-bold text-slate-900 dark:text-white'>Pengeluaran Perbulan</h1>
          <p className='text-slate-500 dark:text-slate-400'>Rincian belanja berdasarkan kode anggaran Tahun {tahunAktif}</p>
        </div>
        <div className='flex items-center gap-2'>
          <ReportPrintButton title={`Pengeluaran_Perbulan_${MONTHS[selectedBulan - 1]}_${tahunAktif}`} />
      ...
      <ReportPrintDocument
        ...
        meta={[
          ...
          { label: 'Total Pengeluaran', value: formatCurrency(rawMonthTotal) },
        ]}
      >
        <table>
          <thead>
              ...
              ...group.txs
                ...
                .map((tx, index) => (
                  <tr key={`${group.kode}-${tx.id}`}>
                    ...
                    <td>{tx.uraian}</td>
                    <td className='text-right'>{formatCurrency(Number(tx.pengeluaran || 0))}</td>
                  </tr>
                )),
      ...
      <div className='grid gap-3 md:grid-cols-2 xl:grid-cols-4'>
        <Card>
          <CardContent className='p-4'>
            ...
            <p className='mt-2 text-xl font-bold text-rose-700 dark:text-rose-300'>{formatCurrency(rawMonthTotal)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className='p-4'>
        ...
        <CardHeader className='gap-4 lg:flex-row lg:items-center lg:justify-between'>
          <div>
            ...
            <p className='text-sm text-slate-500 dark:text-slate-400'>
              Menampilkan {groupedByKode.length} kode dari {monthlyRows.length} transaksi pengeluaran.
            </p>
          </div>
        ...
        <div className='space-y-4'>
          {groupedByKode.map((group) => (
            <Card key={group.kode}>
              <CardHeader className='gap-3 pb-2 lg:flex-row lg:items-start lg:justify-between'>
                ...
                <div className='text-right text-lg font-bold text-rose-700 dark:text-rose-300'>{formatCurrency(group.total)}</div>
              </CardHeader>
              <CardContent>
          ...
          <Card ref={bottomTotalRef} className='bg-slate-900 text-white'>
            <CardContent className='flex items-center justify-between gap-4 py-4'>
              <span className='text-base font-bold lg:text-lg'>TOTAL PENGELUARAN {MONTHS[selectedBulan - 1].toUpperCase()} {tahunAktif}</span>
      ...
      <div className='pointer-events-none fixed bottom-4 left-0 right-0 z-40 px-4 sm:px-6 lg:left-64 lg:px-8'>
        ...
        >
          <div className='min-w-0'>
            <p className='truncate text-lg font-bold'>TOTAL PENGELUARAN {MONTHS[selectedBulan - 1].toUpperCase()} {tahunAktif}</p>
          </div>
